...
class MF_logistic:
    def __init__(self, evaluation_type, n_factors=256, learning_rate=0.05, n_epochs=35, reg_lambda=5e-5, verbose=True):
    ...
    def fit(self, all_train_data):
        ...
        def train(train_set, verbose=False, validate_set=None, tag="Train"):
            ...
            for epoch in range(self.n_epochs):
                ...
                loss = criterion(pred_result[mask], interaction_matrix[mask])
    ...
    def validate(self, train_set, validate_set):
        ...
        pred_scores = torch.full((len(user_list),), 0.49, dtype=torch.float32, device=self.device)
        ...
        if len(valid_user_idx) > 0:
            ...
            pred_scores[valid_mask] = torch.sigmoid(pred_score)

        criterion = torch.nn.BCELoss()
        loss = criterion(pred_scores, true_labels).item()

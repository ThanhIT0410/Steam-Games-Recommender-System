...
class MF_hinge_pairwise:
        ...
        def train(train_set, verbose=False, validate_set=None, tag="Train"):
            ...
            for epoch in range(self.n_epochs):
                ...
                for start in range(0, len(user_list), self.batch_size):
                    ...
                    user_vec = self.user_embedding[user_idx_tensor]
                    pos_vec = self.game_embedding[pos_idx_tensor]
                    neg_vec = self.game_embedding[neg_idx_tensor]

                    user_bias = self.user_bias[user_idx_tensor]
                    pos_bias = self.game_bias[pos_idx_tensor]
                    neg_bias = self.game_bias[neg_idx_tensor]

                    pos_score = (user_vec * pos_vec).sum(dim=1) + user_bias + pos_bias + self.global_bias
                    neg_score = (user_vec * neg_vec).sum(dim=1) + user_bias + neg_bias + self.global_bias
                ...
                if verbose:
                    ...
                    print(f"[MF-BPR] Epoch {epoch+1}/{self.n_epochs}, Loss: {train_loss:.6f}, {val_loss}")
    ...
    def validate(self, train_set, validate_set):
        ...
        with torch.no_grad():
            ...
            pos_vec = self.game_embedding[pos_idx]
            neg_vec = self.game_embedding[neg_idx]
